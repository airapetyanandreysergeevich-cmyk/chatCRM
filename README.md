...
## Подключение мастерской

Владелец оставляет заявку на `/register` — форма открыта всем. Мастерская при этом **не создаётся**: в системе появляется только строка в `TenantApplication`. Поэтому отклонённая заявка не оставляет за собой ни базы, ни ролей, ни статусов, а открытая форма не даёт боту ничего, кроме записи в таблице.

Собственник платформы видит счётчик новых заявок в панели и одобряет заявку одной кнопкой — тогда разворачивается вся начинка мастерской и создаётся учётка владельца. Пароль владелец задал сам при регистрации: в открытом виде его у нас нет, при одобрении переносится готовый хеш, и человек входит тем же паролем, что придумал.

Код мастерской у заявителя не спрашивается — он подбирается из названия (`Сервис на Ленина` → `servis-na-lenina`), при одобрении его можно задать вручную.

Пока заявка не рассмотрена, попытка входа отвечает её статусом — но только тому, кто знает пароль от заявки. Иначе форма входа выдавала бы, кто к нам обращался.

Внешний канал уведомлений (своё приложение, бот, вебхук) подключается одной реализацией в `src/lib/notify.ts` — вызовы по коду уже расставлены.

## Вход

Форма входа одна для всех: email и пароль. Кто пришёл — сотрудник мастерской или команда платформы — определяет сервер по адресу, выбирать пользователю нечего.

Поэтому email уникален **по всей платформе**, а не внутри мастерской, и одна почта не может вести в две учётные записи. У сотрудника без почты владелец заводит адрес вида `master1@masterskaya.local` — он служит только идентификатором для входа.

Поиск учётной записи по email — единственное место в аутентификации, где снимается изоляция по мастерской: пока пользователь не найден, `tenantId` неизвестен. Этот запрос возвращает ровно два поля, `id` и `tenantId`; пароль проверяется уже внутри `withTenant`. Несуществующий адрес отвечает за то же время, что и существующий, и тем же текстом ошибки.
...
Обычный путь — `deploy/setup-https.sh домен.ру`: Let's Encrypt проверяет владение доменом, скачивая файл с вашего сервера по порту 80.

Если к серверу не проходит входящий трафик из-за рубежа (у части российских хостингов так и есть), эта проверка не сработает никогда: серверы Let's Encrypt стоят в Европе и США. Симптом — `Error getting validation data` при том, что файл отдаётся и локально, и по внешнему адресу с самого сервера. Проверить можно на check-host.net: если российские узлы отвечают, а европейские в таймаут — дело в этом.

Тогда сертификат выпускается через DNS-проверку, которой входящие соединения не нужны вовсе:

```bash
# в deploy/.env добавить TIMEWEB_API_TOKEN=...
bash deploy/setup-https-dns.sh www.example.ru
```

Скрипт сам ставит и убирает TXT-запись через API DNS-провайдера, раскладывает сертификат туда, где его ждёт nginx, переключает конфиг на HTTPS и заводит автопродление в cron (`deploy/renew-cert.sh`). Токен живёт в `deploy/.env`, а не в crontab.
...
Если содержимое `deploy/.env` куда-то попало — в переписку, в скриншот, в чужие руки:
...
Срочнее всего — ключи JWT. Зная `JWT_ACCESS_SECRET`, можно подписать себе токен любого пользователя, включая собственника платформы, и обратиться к API через интернет: доступ к серверу для этого не нужен. Пароли Postgres и MinIO наружу не опубликованы, там риск ниже.

Скрипт меняет ключи, гасит все сессии (refresh-токены — случайные строки, смена ключа их сама по себе не отменяет), переписывает пароли ролей в самой базе и в `.env`, пересоздаёт контейнеры. Старый `.env` остаётся рядом резервной копией.

## Забыли пароль

Экрана смены собственного пароля пока нет. Пароль любой учётной записи меняется с сервера:

```bash
bash deploy/set-password.sh user@example.com
```

Пароль вводится скрыто, в аргументы и историю команд не попадает. Все сессии этой учётной записи гасятся — человека выкинет со всех устройств, это и есть смысл смены.
...
Схема поменялась — новая миграция создаётся сравнением схемы с живой базой, без потери данных:

```bash
bash deploy/db-new-migration.sh add_something
```

Скрипт покажет получившийся SQL перед применением и не создаст пустую миграцию, если различий нет.
Первые миграции на чистой базе делает `deploy/db-init-migrations.sh`, его вызывает `install.sh`.
...
| Метод | Путь | Кто |
| --- | --- | --- |
| POST | `/api/public/register` | заявка на подключение мастерской, без авторизации |
| POST | `/api/auth/login` | вход один на всех: email + пароль |
...
| GET | `/api/platform/summary` | счётчики для панели, включая новые заявки |
| GET | `/api/platform/applications` | заявки на подключение |
| POST | `/api/platform/applications/:id/approve` | одобрить: разворачивает мастерскую целиком |
| POST | `/api/platform/applications/:id/reject` | отклонить с причиной |
