...
class Updater {
  ...
  async offerInstall(info) {
    const now = await this.ui.ask(
      ...
      "Для установки программа закроется на минуту: сделает резервную копию базы, " +
        "остановит сервер и откроет окно установщика — в нём будет видно, как идёт установка. " +
        "После неё программа запустится снова.\n\n" +
  ...
  async install() {
    ...
    // Установщик показываем, а не прячем.
    //
    // Сначала обновление шло молча: казалось, что окно установщика в режиме
    // обновления человеку ничего не даёт, кроме лишнего «Далее». На деле
    // вышло наоборот — программа исчезала с экрана, и следующие две минуты
    // не происходило ровным счётом ничего: ни окна, ни значка в трее, ни
    // полоски. Со стороны это неотличимо от «всё повисло», и человек шёл
    // запускать программу заново, пока установщик переписывал её файлы.
    //
    // Показать что-то своё в эти две минуты нельзя: программы уже нет.
    // Заставку пробовали запускать отдельной программой Windows — на рабочей
    // машине она не появилась. Поэтому полосу прогресса показывает тот, кто
    // единственный в этот момент жив, — сам установщик.
