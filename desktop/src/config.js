...
const DEFAULTS = {
  ...
  /**
   * Раздача базы другим компьютерам локальной сети. Включена всегда: от
   * чужих сетей защищает брандмауэр Windows, правило которого действует только
   * в частных сетях. Поле осталось, чтобы старые настройки читались без
   * ошибок; при запуске Основа включает его сама.
   */
  share: {
    enabled: true,
    port: 7373,
