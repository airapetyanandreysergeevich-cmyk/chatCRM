...
# Самая опасная кнопка в программе. Проверяем не то, что она работает, а то,
# чего она не делает: не трогает деньги и не удаляет родителя раньше детей.
echo ">>> Стирание базы"
docker run --rm -v "$(pwd)/backend:/app" -w /app node:20-alpine sh -c "
  $INSTALL
...
# Кнопки приёмки: без запятых в подписях, флаги гарантии защищены,
# сбой счёта нажатий не роняет приём заказа.
echo ">>> Кнопки быстрого заполнения"
docker run --rm -v "$(pwd)/backend:/app" -w /app node:20-alpine sh -c "
  $INSTALL
