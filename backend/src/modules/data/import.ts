...
/**
 * Сколько строк берём из одного файла.
 *
 * Десять тысяч — это не «сколько влезет», а сколько успевает записаться, пока
 * человек ждёт ответа. Каждая строка заказа — это несколько запросов к базе, и
 * вся загрузка идёт одной транзакцией; справочники мастерской читаются один
 * раз, иначе на четырёх тысячах заказов набегало сорок тысяч запросов и ответ
 * не приходил вовсе.
 *
 * Поднимать выше без нужды не стоит: дело упрётся уже не в это число, а в
 * терпение nginx и в размер транзакции.
 */
