 ...
 *
 * Приёмщик печатает марку и модель руками, и одна и та же «Lenovo IdeaPad 5»
 * набирается по десятому разу. Готового справочника завести нельзя: марок и
 * моделей на свете больше, чем можно перечислить, и у каждой мастерской свой
 * поток техники. Поэтому список не задаётся, а накапливается сам — из того,
 * что уже приняли.
 */
...
export type HintField = "brand" | "model";
...
/**
 * Запоминает марку и модель принятой техники.
 *
